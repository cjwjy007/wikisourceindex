<template>
  <div class="search-container">
    ...
    <div class="search">
      <div class="s-bar">
        <form>
          <input type="text" placeholder="Input Words" v-model="input">
          <input class="submit" value="Search" @click="search">
        </form>
      </div>
      <p>Popular searches: <a href="#">Will,</a> <a href="#"> Young </a></p>
    </div>
    <transition>
      <div v-if="isResultShow">
        <div class="search-card-bar">
          <Card v-for="card in cardInfo" style="display:inline-block; width: 40%;margin: 10px 30px">
            <p slot="title">{{card}}</p>
            <Button type="primary" long @click='goToWiki(card)'>跳转到该页面</Button>
          </Card>
        </div>
      </div>
    </transition>
  ...
  export default {
    ...
    methods: {
      search() {
        ...
        $.ajax({
          url: 'http://localhost:8888/?word=' + this.input,
          type: "get",
          success: (result) => {
            console.log(result);
            this.cardInfo = result.replace('[', '').replace(']', '').split(',');
            this.loading = false;
      ...
      goToWiki(title){
        let url = "https://en.wikisource.org/w/index.php?search="+title;
        window.open(url);
      }
    }
  }
...
<style scoped>
  ...
  .s-bar input[type="text"] {
    ...
    background: rgba(188, 216, 255, 0.40);
    border: none;
    outline: none;
